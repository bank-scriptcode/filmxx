...
function page() {
  ...
  const UiSearch = () => {
    return (
      <>
        <div>
          <div className="text-sm mb-2">Process Type</div>
          <MySelect
            placeholder={
              <div className=" text-sm text-[#cacaca] whitespace-nowrap ">{`Select Process Type`}</div>
            }
        ...
        <div>
          <div className="text-sm mb-2">Nomination Type</div>
          <MySelect
            placeholder={
              <div className=" text-sm text-[#cacaca] whitespace-nowrap ">{`Select Nomination Type`}</div>
            }
            // options={userTypeOption}
            options={(bankData || [])?.map((e: any) => {
              return {
  ...
  useEffect(() => {
    let filter = dataStateTemp.filter((f: any) => {
      return (
        ((f?.processType?.name && String(f?.processType?.name)) || "")
          .toLowerCase()
          .includes(searchField.toLowerCase()) ||
        ((f?.nominationType?.name && String(f?.nominationType?.name)) || "")
          .toLowerCase()
          .includes(searchField.toLowerCase()) ||
        ...
        ((f?.startDate && String(f?.startDate)) || "")
          .toLowerCase()
          .includes(searchField.toLowerCase()) ||
        ((f?.beforeGasDay && String(f?.beforeGasDay)) || "")
          .toLowerCase()
          .includes(searchField.toLowerCase()) ||
                    ...
                    <div>
                      ...
                      <div className="relative">
                        ...
                        <input
                          type="time"
                          id="time"
                          className={`text-sm text-[#cacaca] pl-3 pr-3 py-2 h-[38px] w-full rounded-lg border-[1px] border-[#DFE4EA] bg-white outline-none bg-opacity-100 focus:border-[#2B2A87]`}
                          min="09:00"
                          max="18:00"
                          value={time}
                          onChange={(e:any) => {
