...
let mockFix:any = [
  {
    ...
    searchEnable: true,
    keys: "conceptPoint",
    headValue: `Concept Point`,
    bodyClass: ``,
  },
  {
    sortEnable: true,
    searchEnable: true,
    keys: "typeConceptPoint",
    headValue: `Type Concept Points`,
    bodyClass: ``,
  },
  {
    sortEnable: true,
...
function page() {
  ...
  const UiHeadTable = ({sortEnable, keys, value, data}:any) => {
    ...
    return (
      <>
      {!!sortEnable && !!data ?
          data.map((e: any) => hasAgeKey(e, keys))
          .find((f: any) => {
            return f === true;
          }) && (
          <th
  ...
  const UiSearch = () => {
    ...
    return (
      <>
            <div>
              <div className="text-sm mb-2">Concept Point</div>
              <MySelect
                placeholder={
                  <div className=" text-sm text-[#cacaca] whitespace-nowrap ">{`Select Concept Point`}</div>
                }
            ...
            <div>
              <div className="text-sm mb-2">Type Concept Point</div>
              <MySelect
                placeholder={
                  <div className=" text-sm text-[#cacaca] whitespace-nowrap ">{`Select Type Concept Point`}</div>
                }
                // options={userTypeOption}
                options={(bankData || [])?.map((e: any) => {
                  return {
  ...
  useEffect(() => {
    const sortedData = [...dataStateInit].sort((a: any, b: any) => {
      ...
      let fil = configDataRow.find((f:any) => { return f?.keys ===  sortBy})
  ...
  useEffect(() => {
    let filter = dataStateTemp.filter((f: any) => {
      ...
      return (
        ((f?.conceptPoint && String(f?.conceptPoint)) || "")
          .toLowerCase()
          .includes(searchField.toLowerCase()) ||
        ((f?.typeConceptPoint?.name && String(f?.typeConceptPoint?.name)) || "")
          .toLowerCase()
          .includes(searchField.toLowerCase()) ||
                              ...
                              <div key={ix} className="grid">
                                {
                                  items?.sortEnable &&
                                    <div className="flex items-center gap-2">
                                          ...
                                          onClick={() => {
                                            setDataState((pre: any) => {
                                              let newArr = pre.map((obj: any) => {
                                                ...
                                                let newObj:any = { ...obj };
                                                return {...newObj, [items?.keys]: fil[`${items?.keys}`]};
                                              });
                                              return newArr;
                                            });
                                          }}
                                      ...
                                      <div className=" whitespace-nowrap">{items?.headValue}</div>
                                    </div>
                                }
                            </div>
                            )
                      ...
                      <tr>
                        {
                          configDataRow.map((item:any, ix:any) => {
                            return (
                              <UiHeadTable key={ix} sortEnable={item?.sortEnable} keys={item?.keys} value={item?.headValue} data={dataState} />
                            )
                          })
                        }
                      </tr>
                              ...
                              >
                                {
                                  configDataRow.map((el:any, il:any) => {
                                    ...
                                    return (
                                      <UiBodyTable key={il} sortEnable={el?.sortEnable} keys={el?.keys} className={`${!!el?.keys && "px-6 py-4"} ${el?.bodyClass}`} value={valueShow} data={dataState} />
                                    )
                                  })
                                }
