...
const dataTemp: any = [
  {
    ...
    activity: {
      id: 1,
      name: "TSO reject Weekly Nomination",
    },
    active: true,
    // createBy: {
    //   name: "demo ABC",
  ...
  useEffect(() => {
    let filter = dataStateTemp.filter((f: any) => {
      return (
        ...
        ((f?.activity?.name && String(f?.activity?.name)) || "")
          .toLowerCase()
          .includes(searchField.toLowerCase()) ||
        ((f?.active && String(f?.active)) || "")
          .toLowerCase()
          .includes(searchField.toLowerCase())
                          ...
                          >
                            {configDataRow.map((el: any, il: any) => {
                              ...
                              switch (el?.keys) {
                                ...
                                case "active":
                                  valueShow = <>
                                      <ToggleOnIcon className="text-[#24AB6A] cursor-pointer" />
                                  </>;
                                  break;
                ...
                <>
                  ...
                  <div className=" grid grid-cols-2 gap-3">
                    <div className=" col-span-2">
                      ...
                      <MySelect
                        placeholder={
                          <div className=" text-sm text-[#cacaca] whitespace-nowrap">{`Select Activity`}</div>
                        }
                        // options={userTypeOption}
                        options={(bankData || [])?.map((e: any) => {
                          return {
                  ...
                  <div>
                      ...
                      <div className=" flex items-center gap-2 text-[#58585A]">
                      <CheckBoxIcon className=" text-[#9CA3AF] cursor-pointer" />
                      <div>{`Shipper`}</div>
                      <CheckBoxIcon className=" text-[#9CA3AF] cursor-pointer" />
                      <div>{`Operator`}</div>
                      </div>
